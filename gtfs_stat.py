...
#sys.path.insert(0, r'Y:\champ\util\pythonlib-migration\master_versions\logger')
#import logger
...
class stats():
    ...
    def _setup_distributed_processing(self):
        ...
        #from dispy_processing_utils import meantime, stdtime
        #from dispy_processing_utils import filename_generator
        ...
        self.cluster = dispy.JobCluster(proc_stop_time_stats, 
                                        ...
                                        depends=[meantime, 
                                                 ...
                                                 dump_pickle,
                                                 #dt, 
                                                 #r'Y:\Users\Drew\gtfs_stat_maker\gtfs_stat_maker\gtfs_stat.py'
                                                 #__file__,
                                                 dt
                                                 ],
    ...
    def stop_time_stats(self, service_id=1, groupby=None, stat_args=None):
        if self.distributed:
            print "Distributed mode is not currently working."
            sys.exit(2)
    ...
    def _stop_time_stats_distributed(self, service_id=1):
        # apc data is stored by month (or possibly other chunks)
        i = 0
        for key in self.apc_keys:
            apc = pd.read_hdf(self.apc_path, key, stop=1000000)
            ...
            # get the full possible set of stops associated with a trip
            apc.drop_duplicates(subset=['ROUTE_SHORT_NAME','DIR','PATTCODE','TRIP','SEQ','STOP_AVL','ARRIVAL_TIME'], inplace=True)
            #s = apc.groupby(['ROUTE_SHORT_NAME','DIR','PATTCODE','TRIP','SEQ']).agg({'STOP_AVL':pd.Series.nunique})
            #assert (len(s.loc[s['STOP_AVL'].ne(1)]) == 0) # if this isn't true, then the pivot below won't work
            canonical_trip_seq = apc.groupby(['ROUTE_SHORT_NAME','DIR','PATTCODE','TRIP','SEQ','STOP_AVL'], as_index=False).size().reset_index()
            canonical_trip_seq.rename(columns={0:'samples'}, inplace=True)
            canonical_trip_seq.sort_values(by=['ROUTE_SHORT_NAME','DIR','PATTCODE','TRIP','SEQ'], inplace=True)
            
            agg_columns = ['ROUTE_SHORT_NAME','DIR','PATTCODE','TRIP','SEQ','STOP_AVL']
            #stat_args = {'ARRIVAL_TIME':['min','max','size']}
            #stat_args={'ARRIVAL_TIME':[meantime,'size']}
            stat_args={'ARRIVAL_TIME':[meantime,stdtime,'size']}
            ...
            if self.distributed:
                ...
                job = self.cluster.submit(ifile, ofile, agg_columns, stat_args)
